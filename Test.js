...
//If arrDays isn't empty
    if (arrDays.length != 0){

      //check whether the days are the same or not.
     var nd = new Date().toLocaleDateString('en-us', { weekday:"long"});
     var today = localStorage.getItem("today");
    

      //If the days are not the same
      //run through and uncheck all of arrdays.
      if (nd !== today){
        alert( nd + " " + today)
      ...
      //then make today, newday 
     today = nd;
     localStorage.setItem("today", today);  

      //and transfer arrdays into td.
      for (var i = 0; i < arrDays.length; i++){
        let obj = arrDays[i];
      ...
      //then check to see if arrTrans is empty
      //and if it's not empty, concantenate.
      if (arrTrans != 0){
        td = td.concat(arrTrans);
      }

    //if arrdays is empty
    //Check to see if arrTrans is empty
    //and if not, push arrTrans into td.
    } else {   
      if (arrTrans != 0){
    ...
    //If td is an array, call createCheckbox to create task list.
    if (Array.isArray(td)){
